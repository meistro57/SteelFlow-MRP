<!-- resources/js/Pages/Projects/ImportXsr.vue -->
<script setup>
...
const submit = () => {
    form.post(route('projects.import-xsr.store', props.project.id), {
        forceFormData: true,
    });
};
</script>
...
<template>
  <AppLayout title="Import XSR File">
    <div class="mb-8 flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-steel-400 uppercase tracking-wider font-mono">
          Upload an XSR CAD file to import assemblies and parts for {{ project.name }}
        </p>
      </div>
      <Link
    ...
    >
      <div>
        <label class="input-label">XSR File</label>
        <input
          type="file"
          accept=".txt,.csv,.xsr"
          class="input"
        ...
        <p class="mt-2 text-xs text-steel-400 font-mono">
          XSR format files contain assembly and part information from CAD software.
          Expected CSV format with headers: AssemblyMark, PartMark, Quantity, Material, Shape, Length, Weight.
        </p>
        <p class="mt-2">
          <a
            :href="route('import-templates.xsr')"
            class="text-sm text-forge-400 hover:text-forge-300 font-mono underline"
          >
            Download Template XSR File
          </a>
        </p>
      </div>
      ...
      <div class="flex justify-end space-x-3">
        ...
        >
          ...
          <span v-else>Import XSR File</span>
        </button>
      </div>
    </form>
  </AppLayout>
