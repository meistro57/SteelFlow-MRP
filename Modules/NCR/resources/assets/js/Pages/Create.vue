            ...
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border-t-4 border-red-500">
                    <div class="p-6 bg-gray-50 border-b border-gray-200">
                        <h3 class="text-lg font-bold text-gray-700">New Quality Incident Report</h3>
                        <p class="text-sm text-gray-500">Document the failure details and affected items.</p>
                    </div>
                    ...
                    <form @submit.prevent="submit" class="p-6">
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div v-if="form.type === 'part'">
                                ...
                                <select v-model="form.part_instance_id" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500" required>
                                    <option value="">-- Choose Part Instance --</option>
                                    <option v-for="pi in partInstances" :key="pi.id" :value="pi.id">
                                        {{ pi.part?.part_mark }} - {{ pi.part?.assembly?.mark }} (ID: {{ pi.id }})
                                    </option>
                                </select>
                                <p v-if="!partInstances.length" class="text-xs text-red-500 mt-1">No eligible part instances found.</p>
                            </div>
                            ...
                            <div v-if="form.type === 'production'">
                                ...
                                <select v-model="form.production_item_id" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500" required>
                                    <option value="">-- Choose Production Item --</option>
                                    <option v-for="item in productionItems" :key="item.id" :value="item.id">
                                        {{ item.part_instance?.part?.part_mark || 'Unknown' }} - Batch #{{ item.batch_id }}
                                    </option>
                                </select>
                                <p v-if="!productionItems.length" class="text-xs text-red-500 mt-1">No eligible production items found.</p>
                            </div>
                            ...
                            <div v-if="form.type === 'material'">
                                ...
                                <select v-model="form.stock_item_id" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500" required>
                                    <option value="">-- Choose Stock Item --</option>
                                    <option v-for="si in stockItems" :key="si.id" :value="si.id">
                                        {{ si.stock_id }} - {{ si.material?.type }} {{ si.size }} (H: {{ si.heat_number || 'N/A' }})
                                    </option>
                                </select>
                                <p v-if="!stockItems.length" class="text-xs text-red-500 mt-1">No eligible stock items found.</p>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    required
                                ></textarea>
                                <div v-if="form.errors.failure_reason" class="text-red-500 text-xs mt-1">{{ form.errors.failure_reason }}</div>
                            </div>
                        ...
                        <div class="flex items-center justify-between mt-8 pt-6 border-t border-gray-100">
                            <Link :href="route('ncr.index')" class="text-gray-500 hover:text-gray-700 font-medium text-sm">
                                Cancel & Return
                            </Link>
